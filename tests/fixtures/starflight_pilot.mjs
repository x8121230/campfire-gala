export function starPilot(s){const p=s.player;let target={x:220,y:240};const items=s.pickups.filter(a=>a.x<750&&a.x>80).sort((a,b)=>Math.hypot(a.x-p.x,a.y-p.y)-Math.hypot(b.x-p.x,b.y-p.y));
 ...
 const speed=122.5,candidates=[[0,0],[1,0],[-1,0],[0,1],[0,-1],[.707,.707],[.707,-.707],[-.707,.707],[-.707,-.707]];
 ...
 for(const g of s.gates)if(Math.abs(px-(g.x-155*t))<g.w/2+38&&(py<g.gapY-g.gap/2+33||py>g.gapY+g.gap/2-33))cost+=25000;
 if(s.boss?.beam>0&&s.boss.beam-t<1&&Math.abs(py-s.boss.beamY)<58)cost+=20000;
 if(py<40||py>438)cost+=100;if(px<80)cost+=200;
 }if(!best||cost<best.cost)best={x,y,cost};}

 return {x:best.x,y:best.y,fire:true,special:!s.charging&&s.specialCooldowns[s.specialWeapon]<=0,bomb:best.cost>6000&&s.ultimateEnergy>=100&&p.invuln<.1,dash:best.cost>4000&&p.invuln<.1};}
export function playStar(s,limit=900){for(let i=0;i<limit*60&&s.status==='playing';i++){if(s.weaponOffer){const choice=s.weaponOffer.includes('laser')?'laser':s.weaponOffer.includes('homing')?'homing':s.weaponOffer[0];s.chooseWeapon(choice);}s.advance(1/60,starPilot(s));s.events=[];}return {status:s.status,time:Math.round(s.time),hp:s.player.hp,score:s.score,stats:s.stats,ranks:s.weaponRanks,weapon:s.weapon};}
