...
export function iceEvent(s,event){
 ...
 if(beat==='rabbits'){
  ...
  s.message('雪球兔的泡泡吸收攻擊 · 擊破後攻擊兔子本體');
 }
 if(beat==='swan'){
  const e=s.spawn(25,1380,plan===1?210:135+plan*90,'iceSwan',group);e.fireCD=1.1;s.message('八音盒冰晶天鵝 · 旋轉前奏後灑出六角雪花彈幕');
 }
 if(beat==='leopard'){
  const e=s.spawn(26,1400,420,'iceLeopard',group);e.fireCD=1.4;s.message('菁英毛線雪豹！留意抬爪動作，避開拋來的毛球');s.emit('warning');
 }
 if(beat==='gust'){
...
export function iceHit(s,e,damage=0){
 ...
 if(e.kind===24&&e.snowShield>0){const absorbed=Math.min(damage,e.snowShield);e.snowShield-=absorbed;e.hp=Math.min(e.maxHp,e.hp+absorbed);e.shieldFlash=.16;if(e.snowShield<=0){s.emit('iceBubbleBreak',e.x,e.y);s.message('泡泡護盾破裂 · 現在可以攻擊兔子本體');}}
}
export function iceDefeat(s,e){
 ...
 if(e.kind===26){s.score+=5;s.stats.iceElites=(s.stats.iceElites||0)+1;s.addPickup('gem',e.x,e.y);s.addPickup('weapon',e.x+46,e.y);s.message('雪豹收起毛線玩具！接住結晶與星芽鈴');s.emit('elite',e.x,e.y);}
}
...
export function iceEnemy(s,e,dt){
 if(e.kind<24||e.kind>27)return false;const speed=s.tuning.enemySpeedScale,p=s.player;e.fireCD-=dt;e.attackAnim=Math.max(0,(e.attackAnim||0)-dt);e.shieldFlash=Math.max(0,(e.shieldFlash||0)-dt);
 if(e.kind===24){e.x-=e.speed*speed*dt;e.y=clamp(e.baseY+Math.sin(e.age*1.8+e.id)*20,55,425);e.snowSize=30+Math.max(0,e.snowShield||0)*.65;}
  ...
  e.x=Math.max(1025,e.x-e.speed*speed*dt);e.y=clamp(e.baseY+Math.sin(e.age*1.25)*55,70,400);e.spin=(e.spin||0)+dt*2;
  if(e.swanWindup>0){e.swanWindup-=dt;if(e.swanWindup<=0){
   for(let i=0;i<6;i++){const a=e.spin+i*Math.PI/3;iceBullet(s,e.x-28,e.y-12,a,125,5);s.bullets.at(-1).swanShot=true;}
   const aim=Math.atan2(p.y-e.y,p.x-e.x);for(const off of [-.18,0,.18]){iceBullet(s,e.x-38,e.y-20,aim+off,165,5);s.bullets.at(-1).swanShot=true;}
   e.attackAnim=.45;e.fireCD=2.5;s.emit('iceChime',e.x,e.y);
  }}else if(e.x<1180&&e.fireCD<=0){e.swanWindup=.65;}
 }else if(e.kind===26){
  e.y=420;
  if(e.throwWindup>0){e.throwWindup-=dt;if(e.throwWindup<=0){throwIceYarn(s,e.x-92,e.y-48);e.attackAnim=.25;e.fireCD=3.45;}}
  else{e.x-=e.speed*speed*dt;if(e.x<1180&&e.fireCD<=0){e.throwWindup=.65;s.emit('warning',e.x,e.y);}}
...
export function throwIceYarn(s,x,y){const t=1.35,gravity=210;s.bullets.push({id:++s.id,x,y,vx:(s.player.x-x)/t,vy:(s.player.y-y-.5*gravity*t*t)/t,yarnGravity:gravity,yarnBall:true,ice:true,r:17,visualRadius:22,life:6,age:0,grazed:false});}
export function stepIceYarn(b,dt){b.age+=dt;if(b.rolling){b.x+=b.vx*dt;b.y=438;return;}b.x+=b.vx*dt;b.y+=b.vy*dt+.5*b.yarnGravity*dt*dt;b.vy+=b.yarnGravity*dt;if(b.y>=438&&b.vy>0){b.y=438;if(!b.bounced){b.bounced=true;b.vy=-95;b.vx*=.7;}else{b.rolling=true;b.vy=0;b.vx=Math.sign(b.vx||-1)*150;}}}
