...
export const STAR_PHASE1={version:'0.10.7',craftId:'swift-vanguard',craftName:'巡天雨燕',normalShotCooldown:.5,chargeSeconds:2,playerSpeed:245,visualScale:1.05,launchSeconds:3,guardianSeconds:3};
...
export class StarflightTouch{
 ...
 reset(){this.owner=null;this.origin={...STAR_CONTROLS.joystick};this.axis={x:0,y:0};this.firing=new Set();this.requests={};this.traitHold=null;}
 down(p){const {x,y,id}=p;for(const key of ['special','bomb','trait','dash']){const c=STAR_CONTROLS[key];if(Math.hypot(x-c.x,y-c.y)<=c.r){if(key==='trait')this.traitHold={id,at:performance.now()};else this.requests[key]=true;return;}}
 ...
 up(p){this.firing.delete(p.id);if(this.traitHold?.id===p.id){if(performance.now()-this.traitHold.at>=1000)this.requests.bay=true;else this.requests.trait=true;this.traitHold=null;}if(p.id===this.owner){this.owner=null;this.axis={x:0,y:0};}}
 read(){if(this.traitHold&&performance.now()-this.traitHold.at>=1000){this.requests.bay=true;this.traitHold=null;}return {...this.axis,fire:this.firing.size>0,...this.requests};}
