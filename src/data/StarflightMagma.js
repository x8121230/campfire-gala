...
export function magmaEvent(s,event){
 ...
 if(beat==='tanuki'){const e=s.spawn(34,1390,78+plan*22,'magmaTanuki',group);e.fireCD=1.1;s.message('炭火熱氣球狸貓 · 上方箭頭預警2秒後落石');}
...
export function magmaEnemy(s,e,dt){
 ...
 else if(e.kind===34){e.x=Math.max(1015,e.x-e.speed*speed*dt);e.y=clamp(e.baseY+Math.sin(e.age*1.35)*30,115,230);if(e.x<1190&&e.fireCD<=0){addHazard(s,'magmaCoal',clamp(p.x+180,520,980),-20,{r:21,warn:.9,vx:-25,vy:0});e.fireCD=3.5;e.attackAnim=.7;s.emit('magmaDrop',e.x,e.y);}}
