...
export function lakeEnemy(s,e,dt){
 ...
 if(e.kind===16){
  ...
  else{e.x-=e.speed*speed*dt;e.y=447-Math.sin(Math.min(1,e.age/2.25)*Math.PI)*320;if(e.age>.92&&!e.apexShot){e.apexShot=true;for(const off of [-.34,0,.34])lakeBullet(s,e.x,e.y,Math.PI/2+off,105);}if(e.age>2.25)e.exit=true;}
 }else if(e.kind===18){
  e.x=Math.max(1015,e.x-e.speed*speed*dt);e.y=clamp(e.baseY+Math.sin(e.age*1.7)*72,70,390);
  if(e.x<1160&&e.fireCD<=0){addHazard(s,'lakeOrb',e.x-35,e.y,{r:28,vx:-78,phase:e.id});e.fireCD=e.prismBroken?2.9:2.25;}
 }else{
