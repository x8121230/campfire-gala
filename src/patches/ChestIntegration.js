...
Collection.prototype.openTests=function(){
 ...
 openTests.call(this);
 this.modal?.list?.find(o=>o.text==='測試送裝備')?.setText('裝備與 BOX 測試');
 this.modal?.list?.find(o=>typeof o.text==='string'&&o.text.startsWith('測試會改變目前存檔'))?.setText('測試會改變存檔；BOX 不占用首勝與保底。');
 if(this.modal)this.button(this.modal,385,312,510,48,'獲得 BOX 並觸碰開啟',()=>{
...
function notify(scene,result){
 ...
 const text=result.status==='miss'?`這次未獲得 BOX · 保底進度 ${result.misses}/3`: `BOX 未出現：${result.message}`;
