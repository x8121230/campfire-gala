...
export default class ForestStarflightGame extends AnimalSnackGame{
 ...
 create(){
  ...
  if(Object.values(SKY).some(k=>!this.textures.exists(k))||!this.textures.exists('star_candy_enemies')||!this.textures.exists('star_candy_scenery')||!this.textures.exists('star_cave_enemies')||!this.textures.exists('star_ice_enemies')||!this.textures.exists('star_wetland_enemies')||!this.textures.exists('star_magma_enemies')||!this.textures.exists('star_forest_enemies')||!this.textures.exists('star_crew')||!this.textures.exists('craft_swift_cruise')||!this.textures.exists('craft_swift_maneuver')||!this.textures.exists('craft_swift_combat')||STAR_CRAFTS.some(c=>!this.textures.exists('craft_'+c.id))||MAGIC.some(k=>!this.textures.exists('stg04_'+k))){
 ...
 showTitle(){this.clear();this.mode='title';const c=getStarCraft(this.selectedCraft);this.add.image(640,360,'star_carnival_bg').setDisplaySize(1280,854);this.add.rectangle(640,360,1280,720,0x102d35,.55);
  ...
  this.text(955,574,'紅鍵短按 '+c.trait+'／長按 1 秒開彈艙　U 加速　K 大招',16,'#d9d9ba');this.b(165,655,260,60,'← 遊戲列表',()=>this.leave());
 ...
 drawControls(){const s=this.session,g=this.control;g.clear();const j=this.touch.owner===null?STAR_CONTROLS.joystick:this.touch.origin;
  ...
  if(this.touch.traitHold){const h=STAR_CONTROLS.trait,held=Math.min(1,(performance.now()-this.touch.traitHold.at)/1000);g.lineStyle(7,0xffe58f,.95).beginPath().arc(h.x,h.y,h.r+6,-Math.PI/2,-Math.PI/2+Math.PI*2*held,false).strokePath();}
  ...
  this.bombLabel.setText(s.ultimateEnergy>=100?'大招\n就緒':'大招\n'+Math.floor(s.ultimateEnergy)+'%');this.traitLabel.setText(this.touch.traitHold?'按住開彈艙':s.player.traitCD>0?s.craft.trait+'\n'+s.player.traitCD.toFixed(1)+'s':s.craft.trait+'\n短按技能\n長按彈艙');
 }
