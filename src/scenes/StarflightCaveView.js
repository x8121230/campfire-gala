...
export function drawCaveWorld(scene,g){
 ...
 for(const h of s.hazards){if(!h.kind.startsWith('cave'))continue;const active=h.age>=h.warn,q=Math.min(1,h.age/Math.max(.01,h.warn));
  ...
  if(h.kind==='caveCrystal'){
   if(!active){const pulse=.45+.45*Math.sin(q*Math.PI*5);g.fillStyle(0xffcf6e,.10+q*.2).fillEllipse(h.x,455,70+q*45,20+q*9);g.lineStyle(5,0xffdfa0,.48+pulse).lineBetween(h.x,0,h.x,450);g.lineStyle(3,0xff866e,.65+q*.25).strokeCircle(h.x,445,24+q*18);for(let i=0;i<4;i++)g.fillStyle(0xffecae,.65).fillTriangle(h.x-20+i*13,8,h.x-15+i*13,28+q*16,h.x-10+i*13,8);
   }else{const trail=Math.min(150,Math.abs(h.vy||420)*.18);g.fillStyle(0xc8b4ff,.16).fillEllipse(h.x,h.y-trail*.45,h.r*1.9,trail);g.fillStyle(0xb9a3ef,.96).fillTriangle(h.x-h.r,h.y-h.r,h.x+h.r,h.y-h.r,h.x,h.y+h.r*1.85);g.fillStyle(0x8beee8,.5).fillTriangle(h.x-h.r*.35,h.y-h.r*.8,h.x+h.r*.15,h.y-h.r*.7,h.x,h.y+h.r*1.25);g.lineStyle(3,0xf6ffff,.85).lineBetween(h.x-5,h.y-h.r+4,h.x,h.y+h.r);}}
