...
export default class DawnDandelionHillsGame extends AnimalSnackGame {
  ...
  preload() {}
  update() {}
  init(data) { this.returnScene = data?.returnScene || 'WorldAtlas'; }
  ...
  create() {
    ...
    import('../realm/DandelionHillsApp.js').then(({ DandelionHillsApp }) => {
      if (this.hillsDead) return;
      this.root.textContent = '';
      this.app = new DandelionHillsApp(this.root, {
        onExit: () => this.scene.start(this.returnScene),
        onTravel: (target) => this.scene.start(target === 'camp' ? 'RealmWorldGame' : this.returnScene, { returnScene: this.returnScene, entry: target === 'camp' ? 'northGate' : '' }),
        onSound: (name) => {
          if (!preferences.value.sfx) return;
          const context = this.sound.context;
          if (context?.state === 'suspended') context.resume().then(() => { if (!this.hillsDead) this.tone(name); });
          else this.tone(name);
        }
      });
      this.app.start();
