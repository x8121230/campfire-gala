import {MAGMA_PLANS,magmaPlan} from '../data/StarflightMagma.js';

const FRAMES={32:'magmaSlime',33:'magmaSalamander',34:'magmaTanuki',35:'magmaPopbird'};
const SIZES={32:112,33:176,34:162,35:94};
...
export function drawMagmaEnemy(scene,g,e,live){
 const id='magma-'+e.id;let v=scene.views.get(id);if(!v){v=scene.add.image(e.x,e.y,'star_magma_enemies',FRAMES[e.kind]||'magmaSlime');scene.entities.add(v);scene.views.set(id,v);}live.add(id);
 ...
 v.setPosition(e.x,e.y+bob).setDisplaySize(width,width*v.frame.height/v.frame.width/1.27).setRotation(e.kind===35?Math.sin(e.age*7)*.1:e.kind===32?Math.sin(e.age*4)*.05:0);
 if(e.flash>0)v.setTintFill(0xffffff);else if(e.kind===35&&(e.heat||0)<.3)v.setTint(0xff7b55);else v.clearTint();
 ...
 if(e.kind===35){const q=Math.max(0,Math.min(1,(e.heat||0)/.65));g.lineStyle(3,0xffe49c,.55).beginPath().arc(e.x,e.y,31,-Math.PI/2,-Math.PI/2+Math.PI*2*(1-q),false).strokePath();}
 bar(g,e);
}
...
export function drawMagmaWorld(scene,g){
 ...
 for(const h of s.hazards){if(!h.kind.startsWith('magma'))continue;const active=h.age>=h.warn,q=Math.min(1,h.age/Math.max(.01,h.warn));
  ...
  if(h.kind==='magmaCoal'){if(!active){g.lineStyle(4,0xff7652,.4+q*.55).strokeCircle(h.x,438,22+q*13);g.fillStyle(0xffc05e,.18+q*.25).fillCircle(h.x,438,11+q*8);}else{g.fillStyle(0x593536).fillCircle(h.x,h.y,h.r+3);g.fillStyle(0xff9b45,.85).fillCircle(h.x+3,h.y+3,h.r*.65);g.lineStyle(5,0xffd081,.4).lineBetween(h.x-(h.vx||0)*.12,h.y-(h.vy||0)*.12,h.x,h.y);}}
