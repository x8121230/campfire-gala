...
function pauseMainGame(){const game=window.forestGame;paused=game?game.scene.getScenes(true).filter(s=>!s.scene.isPaused()).map(s=>s.scene.key):[];for(const key of paused)game.scene.pause(key);game?.sound.pauseAll();}
function launchFolder(folder){const entry=entries.find(item=>item[0]===folder);if(!entry)return false;const [target,title]=entry;list.style.display='none';nav.hidden=true;panel.querySelector('p').hidden=true;back.hidden=false;frame?.remove();frame=document.createElement('iframe');frame.title=title;frame.src='./standalone/'+target+'/index.html';panel.append(frame);return true;}
...
open.onclick=()=>{pauseMainGame();panel.hidden=false;showList();};
...
window.openForestStandalone=(folder)=>{pauseMainGame();panel.hidden=false;if(!launchFolder(folder)){showList();return false;}return true;};
