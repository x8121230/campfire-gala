...
export function updateResourceIcons(icons, hp, mana,maxHp=3,maxMana=3) {
  for (const [kind, value] of [['hp', hp], ['mana', mana]]) {
    const max=kind==='hp'?maxHp:maxMana;const group=icons[kind];while(group.nodes.length<Math.min(8,max)){const node=group.nodes[0].cloneNode(true);group.group.append(node);group.nodes.push(node);}
    group.nodes.forEach((n,i)=>n.style.display=i<Math.min(8,max)?'':'none');
    const count = Math.max(0, Math.min(max, Math.floor(value || 0)));
    if(max>8){if(!group.counter){group.counter=document.createElement('span');group.group.append(group.counter);}group.counter.textContent=`${count}/${max}`;}
    icons[kind].group.setAttribute('aria-label', `${icons[kind].label} ${count}/${max}`);
