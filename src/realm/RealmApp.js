...
import { RealmWorld } from './MentorCampWorldV319.js';
...
export class RealmApp {
  constructor(root, { onExit = () => {}, onTravel = () => {}, onSound = () => {}, entry = '', portalArrival = false } = {}) {
    ...
    this.inventoryState = mentorReward(loadStarsproutInventory(),this.journey.stage);
    this.inventoryState = saveStarsproutInventory(this.inventoryState);
    this.skillResource=loadSkillResource(souvenirProgress(this.inventoryState.souvenirs));
    this.journey.moveMultiplier=souvenirProgress(this.inventoryState.souvenirs).moveMultiplier;
    if(this.skillResource.hp<=0)this.skillResource.hp=this.skillResource.maxHp;
  ...
  tick(time) {
    ...
    if (!this.paused) {
      ...
      if(autoConsume(this.inventoryState,this.skillResource,dt)){this.save();this.notify('甜蘋果 −1 · HP +1');}
      axis = {
  ...
  hud() {
    ...
    this.spStatus.textContent = `SP ${sp.mana}/${sp.maxMana}${sp.mana < sp.maxMana ? ' · ' + (5-sp.spRegenElapsed).toFixed(1) + ' 秒後回復' : ' · 魔力充足'}`;
    this.objective.textContent = 'Lv.'+souvenirProgress(this.inventoryState.souvenirs).level+' · '+this.journey.objective();
    this.inventory.textContent = (this.inventoryState.learnedSkills.includes('slash')?'斬擊 ✓':'尚未學會斬擊')+' · HP '+sp.hp+'/'+sp.maxHp+' · 甜蘋果 '+(this.inventoryState.items['甜蘋果']||0);
  ...
  save() {
    try {
      ...
      this.inventoryState = mentorReward(this.inventoryState,this.journey.stage);
      this.inventoryState = saveStarsproutInventory(this.inventoryState);
      this.storageOK = true;
    } catch {
  ...
  backpack() {
    ...
    this.inventoryState = mentorReward(this.inventoryState,this.journey.stage);
    this.inventoryPanel = new StarsproutInventoryPanel(this.root, { state: this.inventoryState, onChange: (state) => { this.inventoryState = saveStarsproutInventory(state); renderQuickSlots(this); }, onUse: (name) => this.useInventoryItem(name), onNotice: (message) => this.notify(message), onClose: (state) => { this.inventoryState = saveStarsproutInventory(state); renderQuickSlots(this); this.inventoryPanel = null; this.paused = false; this.last = performance.now(); } });
  }
  ...
  useInventoryItem(name) {
    const health=this.skillResource;
  ...
  result(result) {
    ...
    if (result.title) {
      ...
      if (result.win) this.el('div', 'reward', '獎勵：斬擊（SP 0） · 甜蘋果 ×3', card);
