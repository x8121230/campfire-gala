...
export class DandelionHillsApp {
  constructor(root, { onExit = () => {}, onTravel = () => {}, portalArrival = false, onSound = () => {} } = {}) {
    ...
    this.inventoryState=mentorReward(this.inventoryState,campStage);
    const stats=souvenirProgress(this.inventoryState.souvenirs);Object.assign(this.journey,stats,loadSkillResource(stats));if(this.journey.hp<=0)this.journey.downed=true;
    ...
    this.journey.inventory=this.inventoryState.items;this.journey.learnedSkills=this.inventoryState.learnedSkills;
    this.buildUI();
  }
  ...
  buildUI() {
    ...
    this.quickSlots=buildQuickSlots(this);
  ...
  bindInput() {
    ...
    this.on(window, 'keydown', (event) => {
      ...
      if (/^Digit[1-4]$/.test(event.code) && !this.collisionEditing) { this.useQuick(Number(event.code.slice(-1))-1); return; }
  ...
  async start() {
    ...
    this.world.render(this.journey, 0, { x: 0, y: 0 }, false);
      ...
      if (!welcomed) this.dialog('晨曦蒲公英丘陵', '星光棒凝聚斬擊，向前劈下時才會命中。斬擊完整結束前無法移動或防禦。\n\n不攻擊時，面向來襲方向會自動格擋；盾牌耐久歸零將原地暈眩2秒。\n\n波波鼠不會還手，先試著靠近牠練習吧。', [['開始試煉', () => {
  ...
  tick(time) {
    ...
    if (!this.paused) {
      ...
      if(autoConsume(this.inventoryState,this.journey,dt)){this.journey.inventory=this.inventoryState.items;this.save();this.notify('甜蘋果 −1 · HP +1');}
  ...
  hud() {
    const j = this.journey; this.objective.textContent = 'Lv.'+souvenirProgress(this.inventoryState.souvenirs).level+' · '+j.objective(); updateResourceIcons(this.resourceIcons, j.hp, j.mana,j.maxHp,j.maxMana);
    ...
    this.swordLabel.textContent = `斬擊 SP 0${j.attackCooldown > 0 ? '\n' + j.attackCooldown.toFixed(1) + 's' : ''}`;
    this.spStatus.textContent = `SP ${j.mana}/${j.maxMana}${j.mana < j.maxMana ? ' · ' + Math.max(0, 5 - j.spRegenElapsed).toFixed(1) + ' 秒後回復' : ' · 魔力充足'}`;
    this.attackButton.hidden=!j.learnedSkills?.includes('slash');
  ...
  useQuick(index) {
    if(this.paused||this.loading||this.collisionEditing||this.traveling)return;
    const name=this.inventoryState.quickSlots[index];if(!name){this.backpack();return;}this.useInventoryItem(name);
  }
  useInventoryItem(name) {
    const health=this.journey;
    const result=consumeItem(this.inventoryState,health,name);
    if(result.used){health.itemUseCooldown=.8;this.save();this.hud();}
    this.notify(result.message);return {...result,state:this.inventoryState};
  }
  ...
  save() { try { saveSkillResource(this.journey); localStorage.setItem(SAVE, JSON.stringify(this.journey.export())); this.inventoryState.items=this.journey.inventory; this.inventoryState.unlockedBeadSlots = Math.max(this.inventoryState.unlockedBeadSlots, this.journey.questStage >= 2 ? 2 : 1); this.inventoryState = saveStarsproutInventory(this.inventoryState); this.journey.inventory=this.inventoryState.items; } catch { this.notify('目前無法保存進度，請先不要關閉頁面。'); } }
  ...
  backpack() {
    ...
    this.inventoryState.items=this.journey.inventory;
    this.inventoryPanel = new StarsproutInventoryPanel(this.root, {
      state: this.inventoryState,
      onChange: (state) => { this.inventoryState = saveStarsproutInventory(state); renderQuickSlots(this); this.journey.inventory=this.inventoryState.items; },
      onUse: (name) => this.useInventoryItem(name), onNotice: (message) => this.notify(message),
      onClose: (state) => { this.inventoryState = saveStarsproutInventory(state); renderQuickSlots(this); this.journey.inventory=this.inventoryState.items; this.inventoryPanel = null; this.paused = false; this.last = performance.now(); }
    });
  }
  journal() {
