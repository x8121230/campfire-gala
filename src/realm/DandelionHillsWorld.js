...
const MOB_HEIGHT = Object.freeze({ mouse: 160 * MOUSE_VISUAL_SCALE, chick: 98, dew: 100, mole: 108, rabbit: 148 });
...
export class DandelionHillsWorld {
  constructor(canvas) {
    ...
    this.images = {};
    this.detailTiles = new HillsDetailTiles(loadImage);
    this.width = 1;
    this.height = 1;
    this.dpr = 1;
    this.time = 0;
  ...
  drawMob(ctx, mob, journey) {
    ...
    if (animated) {
      ...
      if (mob.cast || mob.recovery > 0) drawCombatFrame(ctx, this.images.combatFrames, mob.type === 'mole' ? 0 : 1, combatFrameIndex(mob), size);
      else drawLifeFrame(ctx, this.images[mob.type+'Life'], mob, size);
      ctx.restore();
    }
  ...
  drawEffects(ctx, dt, journey) {
    for (const fx of this.effects) {
      ...
      } else if (fx.kind === 'poof' && ['mole','dew'].includes(fx.type)) {
        ctx.save();ctx.translate(fx.x,fx.y);if(fx.faceLeft)ctx.scale(-1,1);
        ctx.globalAlpha=Math.max(0,Math.min(1,(1-p)/.25));
        drawLifeFrame(ctx,this.images[fx.type+'Life'],{alive:false,deathTime:fx.maxLife-fx.life},fx.type==='mole'?132:122);
        ctx.restore();
  ...
  render(journey, dt, axis, moving) {
    ...
    if (!this.overview) this.detailTiles.draw(ctx,view,this.width,this.height,Math.max(dt,.016));
  ...
  dispose() { this.detailTiles.dispose(); this.images = {}; this.effects = []; this.creatureFrames = null; this.creatureAnimator.states.clear(); this.canvas.width = 1; this.canvas.height = 1; }
}
