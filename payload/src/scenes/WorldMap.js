...
import { getSubmap } from '../data/WorldRegionData.js';
...
export default class WorldMap extends Phaser.Scene {
    ...
    init(data) {
        ...
        this.submapId = data?.submapId || this.registry.get('forest_submap_id') || 'morning_camp';
        this.returnScene = data?.returnScene || (this.submapId ? 'RegionGuide' : 'Start');
        this.registry.set('forest_submap_id', this.submapId);
        ...
        this.submap = getSubmap('forest', this.submapId);
        this.mapDisplayName = this.submap?.name || mapNames[this.mapID] || '未知地點';
        this.mapTextureKey = this.submap?.background || 'current_map';
    }

    preload() {
        if (this.mapTextureKey === 'current_map' && !this.textures.exists('current_map')) {
    ...
    create() {
        ...
        // 背景
        const bg = this.add.image(640, 360, this.mapTextureKey).setInteractive();
        ...
        this.add.image(65, 115, 'btn_back')
            ...
            .on('pointerdown', () => {
                ...
                this.scene.start(this.returnScene, this.returnScene === 'RegionGuide' ? { regionId: 'forest' } : undefined);
            });
    ...
    createLevelIcons() {
        ...
        ].filter((level) => (this.submap?.stageIds || []).includes(level.stageKey));
